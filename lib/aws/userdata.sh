...
docker pull juspaydotin/hyperswitch-router:v1.119.0-standalone
...
docker run -d --env-file .env -p 80:8080 -v `pwd`/:/local/config juspaydotin/hyperswitch-router:v1.119.0-standalone ./router -f /local/config/production.toml


# Installing Hyperswitch control center

docker pull juspaydotin/hyperswitch-control-center:v1.37.7
...
docker run -d --env-file .env -p 9000:9000 juspaydotin/hyperswitch-control-center:v1.37.7
